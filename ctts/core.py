...
from .utils import check_all_assets, select_device, del_all
...
class CTTS:
    def __init__(self, logger=logging.getLogger(__name__)):
        ...
        self.normalizer = Normalizer(
            os.path.join(os.path.dirname(__file__), "homophones_map.json"),
            logger,
        )
        with open(
            os.path.join(os.path.dirname(__file__), "sha256_map.json")
        ) as f:
    ...
    def download_models(
        self,
        model_path: Optional[torch.serialization.FILE_LIKE] = None,
    ) -> Optional[str]:
        self.logger.log(logging.INFO, f"try to load from local: {model_path}")
        if not check_all_assets(Path(model_path), self.sha256_map, update=False):
            self.logger.error("check models in custom path %s failed.", model_path)
            return None
        return model_path
    ...
    def load(
        ...
        checked_model_path = self.download_models(model_path)
        ...
        return self._load(
            ...
            **{
                k: os.path.join(checked_model_path, v)
